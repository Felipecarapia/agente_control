...
export default function ContasPagarPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const [contas, res] = await Promise.all([
        api<ContaPagar[]>(`/api/v1/financeiro/contas-pagar${statusParam}`).catch(() => []),
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (editId) {
        await api(`/api/v1/financeiro/contas-pagar/${editId}`, { method: "PUT", body: JSON.stringify(payload) });
        toast({ title: "Conta atualizada com sucesso!" });
      } else {
        await api("/api/v1/financeiro/contas-pagar", { method: "POST", body: JSON.stringify(payload) });
  ...
  async function handleDelete() {
    if (!deleteId) return;
    try {
      await api(`/api/v1/financeiro/contas-pagar/${deleteId}`, { method: "DELETE" });
  ...
  async function handlePagar() {
    if (!payId) return;
    try {
      await api(`/api/v1/financeiro/contas-pagar/${payId}/pagar`, { method: "POST" });
      toast({ title: "Conta marcada como paga!" });
      setPayId(null);
      load();
    } catch (e: any) {
      toast({ title: "Erro", description: e?.message || "Falha ao pagar", variant: "destructive" });
    }
  }
  ...
  function isVencido(conta: ContaPagar) {
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Gerencie suas despesas e obrigações financeiras</p>
        </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold">{resumo ? formatCurrency(resumo.total_a_pagar) : "..."}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold">{resumo ? formatCurrency(resumo.total_pago) : "..."}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold">{resumo ? resumo.contas_vencidas_pagar : "..."}</p>
              </div>
            </div>
          </CardContent>
        </Card>
                ...
                {list.map((conta) => {
                  ...
                  return (
                    <TableRow key={conta.id} className={vencido ? "bg-red-500/5" : ""}>
                      ...
                      <TableCell className="text-sm">{conta.fornecedor || "-"}</TableCell>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1">
                          {conta.status === "pendente" && (
                            <Button variant="ghost" size="icon" title="Marcar como pago" onClick={() => setPayId(conta.id)}>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editId ? "Editar Conta a Pagar" : "Nova Conta a Pagar"}</DialogTitle>
          </DialogHeader>
      ...
      <AlertDialog open={!!payId} onOpenChange={() => setPayId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              A conta será marcada como paga com a data de hoje.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancelar</AlertDialogCancel>
            <AlertDialogAction onClick={handlePagar}>
