...
const PRESETS = [
  {
    key: "PENDING_CHECK",
    label: "Pendências no projeto",
    message: "Verifique pendências em {projectName} na sua área.",
  },
  {
    key: "URGENT_UPDATE",
    label: "Atualização urgente",
    message: "Precisamos de uma atualização urgente sobre {projectName}.",
  },
  {
    key: "STATUS_TODAY",
    label: "Preciso de status hoje",
    message: "Por favor, envie o status de {projectName} até o final do dia.",
  },
];
...
export function CobrarModal({ open, onOpenChange, projectId, projectName }: CobrarModalProps) {
  ...
  async function loadMembers() {
    setLoading(true);
    try {
      const data = await api<Member[]>(`/api/v1/projetos/${projectId}/members`);
      setMembers(data);
      // Selecionar todos por padrão
      setSelectedMembers(new Set(data.map((m) => m.id)));
      if (data.length === 0) {
        // Se não houver membros, mostrar mensagem
        console.warn("Nenhum membro encontrado no projeto. Adicione tarefas ou um responsável ao projeto.");
      }
    } catch (e) {
  ...
  async function handleNudge() {
    ...
    try {
      ...
      const message = customMessage || preset?.message.replace("{projectName}", projectName) || "";

      await api(`/api/v1/projetos/${projectId}/nudge`, {
        method: "POST",
        body: JSON.stringify({
      ...
      setSuccess(`${selectedMembers.size} notificação(ões) enviada(s)`);
      setTimeout(() => {
        onOpenChange(false);
      }, 2000);
    } catch (e) {
      ...
      if (errorMessage.includes("403") || errorMessage.includes("Requer")) {
        ...
        alertMessage += "• Administrador, ou\n";
        alertMessage += "• Gestor de Projeto\n\n";
        alertMessage += "Peça a um administrador para atribuir um desses cargos a você.";
      } else if (errorMessage.includes("Failed to fetch") || errorMessage.includes("conexão")) {
        alertMessage = "❌ Erro de conexão com o servidor.\n\n";
        alertMessage += "Verifique:\n";
        alertMessage += "1. Se o backend está rodando (porta 8000)\n";
        alertMessage += "2. Se não há problemas de rede\n";
        alertMessage += "3. Se você tem permissão (ADMIN ou PROJECT_MANAGER)\n";
        alertMessage += "4. Se a migration foi executada (alembic upgrade head)";
      } else {
        alertMessage = `Erro: ${errorMessage}\n\n`;
        alertMessage += "Certifique-se de que:\n";
  ...
  async function handleSendDM(userId: string) {
    ...
    try {
      ...
      alert(`Erro: ${errorMessage}\n\nCertifique-se de que:\n1. A migration foi executada (alembic upgrade head)\n2. Você tem permissão (ADMIN ou PROJECT_MANAGER)`);
    } finally {
      setSending(false);
    }
  }
  ...
  const preset = PRESETS.find((p) => p.key === selectedPreset);
  const previewMessage = customMessage || preset?.message.replace("{projectName}", projectName) || "";

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Cobrar membros do projeto</DialogTitle>
          <DialogDescription>
                    ...
                    {PRESETS.map((p) => (
                      ...
                      >
                        ...
                        <div className="text-xs text-muted-foreground mt-1">
                          {p.message.replace("{projectName}", projectName)}
                        </div>
                      </button>
                    ))}
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  {loading ? (
                    ...
                    <div className="p-4 text-center text-sm text-muted-foreground border border-dashed rounded-lg">
                      ...
                      <p>Nenhum membro encontrado neste projeto</p>
                    </div>
                  ) : (
