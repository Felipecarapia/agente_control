-- SQL SCHEMA PARA SUPABASE (MULTI-TENANT)
-- ⚠️ EXECUTE ESTE SCRIPT NO EDITOR SQL DO SUPABASE

-- 1. Limpeza do Banco (Opcional - Remova se não quiser apagar tudo)
...
CREATE TABLE user_productivity_metrics (
	...
	updated_at TIMESTAMP WITH TIME ZONE DEFAULT now(), 
	PRIMARY KEY (id), 
	CONSTRAINT uq_user_month_productivity UNIQUE (user_id, month_year), 
	FOREIGN KEY(user_id) REFERENCES usuarios (id) ON DELETE CASCADE
);
