...
class TaskProperty(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, index=True)
    task_database_id = Column(UUID(as_uuid=True), ForeignKey("task_databases.id", ondelete="CASCADE"), nullable=False, index=True)
...
class TaskPropertyValue(Base):
    ...
    property_id = Column(UUID(as_uuid=True), ForeignKey("task_properties.id", ondelete="CASCADE"), nullable=False, index=True)
