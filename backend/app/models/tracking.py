...
class ProposalSession(Base):
    ...
    proposta_id = Column(UUID(as_uuid=True), ForeignKey("propostas.id", ondelete="CASCADE"), nullable=False, index=True)
...
class ProposalEvent(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, index=True)
    session_id = Column(UUID(as_uuid=True), ForeignKey("proposal_sessions.id", ondelete="CASCADE"), nullable=False, index=True)
    proposta_id = Column(UUID(as_uuid=True), ForeignKey("propostas.id", ondelete="CASCADE"), nullable=False, index=True)
